...
export default function TrustPaymentsShowcaseEditorRoute() {
  ...
  const [eyebrow, setEyebrow] = useState("Trusted checkout");
  ...
  const [trustPointOne, setTrustPointOne] = useState(
    "Verified customer confidence",
  );
  const [trustPointTwo, setTrustPointTwo] = useState(
    "Secure payment support",
  );
  const [trustPointThree, setTrustPointThree] = useState(
    "Delivery partner visibility",
  );

  const [paymentOne, setPaymentOne] = useState("Visa");
  const [paymentTwo, setPaymentTwo] = useState("Mastercard");
  const [paymentThree, setPaymentThree] = useState("PayPal");
  const [paymentFour, setPaymentFour] = useState("DHL");
